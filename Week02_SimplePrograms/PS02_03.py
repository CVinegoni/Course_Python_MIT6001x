# Problem 3 - Using Bisection Search to Make the Program Faster 20/20 points
# (graded) You'll notice that in Problem 2, your monthly payment had to be a
# multiple of $10. Why did we make it that way? You can try running your
# code locally so that the payment can be any dollar and cent amount (in
# other words, the monthly payment is a multiple of $0.01). Does your code
# still work? It should, but you may notice that your code runs more slowly,
# especially in cases with very large balances and interest rates. (Note:
# when your code is running on our servers, there are limits on the amount
# of computing time each submission is allowed, so your observations from
# running this experiment on the grading system might be limited to an error
...
#  bisection search! lajdf l;ajdsk l;akdsj fl;kasdj f;lkasdj
#  f;laksdjf;laskdjf;lasdkjf;lasdkjf ;lasdkjf ;alsdkjf ;alsdkjf The
#  following variables contain values as described below:
...
# must pay at least this much every month. One-twelfth of the original
...
# numberMonths = 12              # Number of months over which to calculate
#                                   the balance
...
def calculateLowestPayment(balance, lowerBoundPayment, upperBoundPayment):
    """
    function to calculate lower payment to extinguish debt in recursive way,
    with function calling itself. Use search and divide To make it more
    generalizable it expects these variables in the global scope:
    (annualInterestRate, monthlyPaymentRate, balance)

    Args: balance (float): balance of the account at month zero
    ...
    Returns: float: the minimum payment to do
    """
